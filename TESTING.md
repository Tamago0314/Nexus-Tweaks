...
2. スロット 1（消耗 1400 のツルハシ）を持って **空** に向けて右クリック
...
6. ブロック・動物・額縁などに向けて右クリック → 修理されず、バニラの動作（設置など）になる
7. `/function nexus_test:xp5pt` → `/function nexus_test:wear` → 右クリック
   → 耐久が **10 だけ** 回復し、経験値が 0 になる
8. `/function nexus_test:xp0` → 右クリック → 「経験値が足りません」、何も消費されない
9. Config で Auto Repair を OFF → 右クリックしても何も起きない。
   Auto Repair にホットキーを割り当てれば、押すたびに ON/OFF が切り替わってメッセージが出る
10. 右クリック押しっぱなし → 連打にならず、4 tick ごとの実行にとどまる
11. `Nexus-Sync/run/config/nexus-sync.json` の `enabled` を `false` にしてサーバー再起動
    → 右クリックで「このサーバーでは Auto Repair が無効にされています」

### C. 純バニラサーバー（`runClientVanilla`）
1. `/function nexus_test:kit`
2. スロット 1 で空に向けて右クリック
   → 「このサーバーには Nexus-Sync が入っていないため、Auto Repair は使えません」と出て、
     耐久も経験値も変わらない（`/function nexus_test:status` で確認）

### D. シングルプレイ（`runClient`）
1. ワールド作成時に「チートの許可: オン」にする
2. `/function nexus_test:kit` → スロット 1 で空に向けて右クリック → B-2 と同じ結果
